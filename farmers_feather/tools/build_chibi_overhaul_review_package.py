...
BOOK_PATH = PACKAGE_DIR / "FarmersFeather_chibi_overhaul_review.ecbmps"
...
def write_pages(authoring: dict, environment: dict, player: dict, equipment: dict, daemons: dict) -> list[str]:
    page1_lines = [
        "Farmer's Feather Chibi Overhaul Review",
        "",
...
def compile_book(page_args: list[str], compiler_path: Path) -> None:
    ...
    command = [
        ...
        str(BOOK_PATH),
        "-t",
        "Farmer's Feather Chibi Overhaul",
        "-a",
        "GitHub Copilot",
    ] + page_args
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the Farmer's Feather chibi-overhaul review package.")
