...
def write_coverage(manifest: dict[str, object], out_dir: Path) -> Path:
    lines = ["# Farmer's Feather Audio Coverage", "", "## Music", ""]
...
def generate(manifest_path: Path, out_dir: Path, sample_rate: int) -> dict[str, object]:
    ...
    for track in manifest["music_tracks"]:
        ...
        report["music"].append({"id": track["id"], "path": str(path), "frames": len(frames), "trigger": track["trigger"]})

    for item in manifest["sfx"]:
        frames = render_sfx(item, sample_rate)
        path = out_dir / "sfx" / f"{item['id']}.wav"
        write_wave(path, frames, sample_rate)
        report["sfx"].append({"id": item["id"], "path": str(path), "frames": len(frames), "trigger": item["trigger"]})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Farmer's Feather placeholder audio assets.")
