...
In Farmer's Feather it is used to restyle player and daemon kin sprites without replacing the current 8x8 renderer. `PROGHONORAI` supplies the render directive, and `PxGBPROG` handles the vector-oriented scene build, local real-space simulation, and final pixel raster.