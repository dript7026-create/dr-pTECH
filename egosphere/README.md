...
python tools/validate_pipeline.py --suite all
python tools/validate_pipeline.py --gui
```
...
- `tools/validate_pipeline_app.pyw` is a Windows-friendly launcher for opening the validator without a console window.
- `make validate-pipeline`, `make validate-pipeline-all`, and `make validate-pipeline-gui` expose the same checks through the Makefile.
