"""Small HTTP server for the CogniNeueroHub browser application."""
...
from .model import (
    animation_library_payload,
    course_profile_from_payload,
    Directive,
    bootstrap_state,
    dataset_payload,
    default_lesson_payload,
    generate_student_lesson_note,
    lesson_library_payload,
    lesson_profile_from_payload,
...
LESSON_ANIMATION_ROOT = WEB_ROOT / "lesson-animations"
...
def resolve_lesson_animation_asset(request_path: str) -> Path | None:
    relative = request_path.removeprefix("/lesson-animations/")
    candidate = (LESSON_ANIMATION_ROOT / unquote(relative)).resolve()
    try:
        candidate.relative_to(LESSON_ANIMATION_ROOT.resolve())
    except ValueError:
        return None
    if not candidate.exists() or not candidate.is_file():
        return None
...
class CogniNeueroHubHandler(BaseHTTPRequestHandler):
    server_version = "CogniNeueroHub/0.1"
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/api/lessons":
            self._json(lesson_library_payload())
            return
        if self.path == "/api/course":
            self._json(asdict(self.server.state.course))
            return
        if self.path == "/api/course/export":
            self._json({
                "course": asdict(self.server.state.course),
                "exported_from": "CogniNeueroHub",
                "version": 1,
            })
            return
        if self.path == "/api/animations":
            self._json(animation_library_payload())
            return
        if self.path == "/api/assets":
            self._json(build_asset_manifest())
            return
        if self.path.startswith("/lesson-animations/"):
            asset = resolve_lesson_animation_asset(self.path)
            if asset is None:
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/step":
            ...
            if raw_course is not None:
                self.server.state.course = course_profile_from_payload(raw_course)
            raw_lesson = payload.get("lesson")
            if raw_lesson is not None:
                self.server.state.lesson = lesson_profile_from_payload(raw_lesson)
                step_state(self.server.state, steps=0)
        ...
        if self.path == "/api/respond":
            ...
            analysis = score_lesson_response(self.server.state, response_text)
            history_entry = record_student_lesson_history(
                self.server.state,
                str(payload.get("student_id", "")).strip() or None,
                response_score=analysis.score,
            )
            self._json({
                "lesson_id": self.server.state.lesson.lesson_id,
                "response": response_text,
                "analysis": analysis.__dict__,
                "history": asdict(history_entry) if history_entry is not None else None,
            })
            return
        if self.path == "/api/course/setup":
            self.server.state.course = course_profile_from_payload(payload.get("course") or payload)
            self._json(asdict(self.server.state.course))
            return
        if self.path == "/api/course/import":
            raw_course = payload.get("course") or payload
            self.server.state.course = course_profile_from_payload(raw_course)
            self._json(asdict(self.server.state.course))
            return
        if self.path == "/api/student-note":
            raw_lesson = payload.get("lesson")
            if raw_lesson is not None:
                self.server.state.lesson = lesson_profile_from_payload(raw_lesson)
                step_state(self.server.state, steps=0)
            ...
            if raw_course is not None:
                self.server.state.course = course_profile_from_payload(raw_course)
            try:
                note = generate_student_lesson_note(self.server.state, str(payload.get("student_id", "")).strip() or None)
            except ValueError as error:
                self._json({"error": str(error)}, status=400)
                return
            self._json(asdict(note))
            return
        if self.path == "/api/reset":
            seed = int(payload.get("seed", 11))
            agent_count = int(payload.get("agent_count", 18))
            habitat_count = int(payload.get("habitat_count", 4))
            self.server.state = bootstrap_state(
                seed=seed,
                agent_count=agent_count,
                habitat_count=habitat_count,
                lesson=payload.get("lesson") or default_lesson_payload(),
                course=payload.get("course") or asdict(self.server.state.course),
            )
            self._json(self.server.state.to_dict())
            return
        self._json({"error": "not_found"}, status=404)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the CogniNeueroHub local server")
    ...
    server = ThreadingHTTPServer((args.host, args.port), CogniNeueroHubHandler)
    server.state = bootstrap_state(
        seed=args.seed,
        agent_count=args.agents,
        habitat_count=args.habitats,
        lesson=default_lesson_payload(),
    )
    print(f"CogniNeueroHub listening on http://{args.host}:{args.port}")
    try:
