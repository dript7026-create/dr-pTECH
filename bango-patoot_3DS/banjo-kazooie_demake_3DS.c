...
static const char *k_xp_names[XP_VARIETY_COUNT] = {
	"Combat", "Exploration", "Diplomacy", "Arcana", "Survival", "Craft"
};
...
static void build_landscape_for_world(int world_index)
{
	...
	g_game.landscape.width = 18;
	g_game.landscape.height = 18;
...
static void init_worlds(void)
{
	...
	g_game.current_world = BANGO_WORLD_COUNT - 1;
}
...
static void trigger_move(void)
{
	...
	if (g_game.bango_rig_state.rig && g_game.bango_rig_state.rig->imported_pose_count > 0) {
		g_game.bango_rig_state.active_pose = g_game.bango_rig_state.rig->poses[0];
...
static void update_character_rig(CharacterRigState *state, Vec3 root_position, int angle_index, float dt_ms, int locomotion_active)
{
	...
	for (i = 0; i < state->rig->bone_count && i < BANGO_MAX_RUNTIME_BONES; ++i) {
		...
		state->bones[i].position.x = root_position.x + (local_x + offset_x) * cos_yaw;
		state->bones[i].position.z = root_position.z + (local_x + offset_x) * sin_yaw;
		state->bones[i].position.y = root_position.y + local_y + offset_y;
		state->bones[i].rotation = offset_rot;
	}
}
...
static u32 landscape_color_for_cell(int texture_layer, float avg_height, float horror)
{
	int red;
	int green;
	int blue;
	switch (texture_layer) {
	case 2:
		red = 90 + (int)(avg_height * 28.0f) + (int)(horror * 40.0f);
		green = 72 + (int)(avg_height * 22.0f);
		blue = 98 + (int)(horror * 32.0f);
		break;
	case 1:
		red = 76 + (int)(horror * 22.0f);
		green = 92 + (int)(avg_height * 26.0f);
		blue = 74 + (int)(horror * 18.0f);
		break;
	default:
		red = 52 + (int)(horror * 18.0f);
		green = 58 + (int)(avg_height * 12.0f);
		blue = 66 + (int)(horror * 28.0f);
		break;
	}
	return C2D_Color32((u8)clampf((float)red, 0.0f, 255.0f), (u8)clampf((float)green, 0.0f, 255.0f), (u8)clampf((float)blue, 0.0f, 255.0f), 255);
}
