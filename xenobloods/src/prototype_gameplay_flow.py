...
from xenobloods_systems import Alignment, LifeForm, Plane, PlayerState
...
class LowPuzzleState:
    ...
    return_zone: str = "veinmarket"
    step_sequence: tuple[str, ...] = ("refract", "resist", "collapse")
    last_resolution: str = ""
...
class GameplayPrototypeController:
    ...
    def route_to_up_dialogue(self, npc_id: str = "opal_tetrarch") -> None:
        ...
        if self.player.life_form == LifeForm.LANDBORNE:
            self.player.plane = Plane.UP
            self.player.alignment = Alignment.GRAMATOS
            self.player.sync_derived_stats()
        else:
    ...
    def route_to_low_puzzle(
        self,
        curgz_id: str = "curgz_alpha",
        target_progress: int = 3,
        return_zone: str = "veinmarket",
        step_sequence: tuple[str, ...] = ("refract", "resist", "collapse"),
    ) -> None:
        ...
        if self.player.life_form == LifeForm.LANDBORNE:
            self.player.plane = Plane.LOW
            self.player.alignment = Alignment.SORROW
            self.player.sync_derived_stats()
        else:
            self.player.route_from_shrine(Plane.LOW)
        self.current_actor_id = curgz_id
        self.low_puzzle_state = LowPuzzleState(
            curgz_id=curgz_id,
    ...
    def resolve_boss_exchange(self, action: str, timing: float, lane: str, ranged: bool = False) -> None:
        ...
        self.status_text = self.battle_state.last_resolution
        if self.battle_state.enemy_health <= 0.0:
            self.route_to_up_dialogue("auditor_sal")
            if self.dialogue_state is not None:
                self.dialogue_state.target_exchanges = 2
                self.dialogue_state.safe_card = "answer"
            self.status_text = "Lahgroid falls. The difficulty crest breaks into an Up denouement with Auditor Sal."
        elif self.battle_state.player_health <= 0.0:
            self.player.die()
    ...
    def redirect_curgz_current(self, route: str) -> None:
        if self.low_puzzle_state is None or self.mode != GameplayMode.LOW_PUZZLE:
            return
        sequence = self.low_puzzle_state.step_sequence or ("refract", "resist", "collapse")
        target = sequence[self.low_puzzle_state.progress % len(sequence)]
        if route == target:
        ...
        if self.low_puzzle_state.progress >= self.low_puzzle_state.target_progress:
            return_zone = self.low_puzzle_state.return_zone
            self.enter_land_navigation(return_zone)
            if return_zone == "veinmarket":
                self.status_text = "The curgz triad collapses safely. Ishtasha finds a route back toward Land."
            else:
                self.status_text = f"The curgz epilogue settles. Ishtasha rises back toward {return_zone.replace('_', ' ')}."
