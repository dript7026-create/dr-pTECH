// filepath: c:\Users\rrcar\Documents\drIpTECH\skazka\skazavR.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
void addEntity(GameWorld *world, Vec3D pos, NPCType type, const char *name, int level, int lootId) {
    ...
    e->lootId = lootId;
    copyString(e->name, sizeof(e->name), name);
    e->isAlly = 0;
}
...
void addQuest(GameWorld *world, const char *title, const char *desc, int targetEntity, int expReward, int goldReward) {
    ...
    copyString(q->title, sizeof(q->title), title);
    copyString(q->description, sizeof(q->description), desc);
...
void levelUp(Player *p) {
    while (p->stats.exp >= p->stats.expToLevel) {
        p->stats.exp -= p->stats.expToLevel;
        p->stats.level++;
        p->stats.expToLevel = (int)(p->stats.expToLevel * 1.2f);
